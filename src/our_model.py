...
class OurModelNew(nn.Module):
    def __init__(self, backbone, is_B_trainable=True, B_init=None, class_num=10, device=torch.device('cuda')):
        super(OurModelNew, self).__init__()
        self.normalized_loss = nn.BCELoss(reduction='sum')
        self.cluster_projector = nn.Sequential(OrderedDict([\
                ('backbone', backbone), \
                ('bridge', nn.Linear(1000, class_num)), \
...
class OurModelHacking(nn.Module):
    def __init__(self, backbone, is_B_trainable=True, B_init=None, class_num=3, device=torch.device('cuda')):
        super(OurModelHacking, self).__init__()
        self.normalized_loss = nn.BCELoss(reduction='sum')
        self.cluster_projector = nn.Sequential(OrderedDict([\
                ('backbone', backbone), \
                ('bridge', nn.Linear(128, 3)), \
                ('final_softmax', nn.Softmax(dim=1))\
                ]))
        ...
        # self._B = torch.from_numpy(np.array(
        #         [[0.9, 0.5, 0.9],
        #          [0.1, 0.9, 0.5],
        #          [0.4, 0.05, 0.9]]).astype(np.float32)).to(device)

        self._sigmoid = lambda x: x
        self.I = torch.eye(class_num).to(device)
        self.xxxx = lambda x: x
        # self.xxxx = nn.Sigmoid()
    ...
    def link_prediction(self, B, c_i, c_j):
        # return torch.mul(torch.matmul(c_i, B), c_j).sum(-1)
        tmp = self.xxxx(torch.mul(torch.matmul(c_i, B), c_j).sum(-1))
        return tmp
